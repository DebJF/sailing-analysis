...
- `currentView`: `'map' | 'beating' | 'twd'`

### Views

- **Map** (`map-container`): Leaflet map with CartoDB light + OpenSeaMap seamark overlay. Each boat gets a polyline + circleMarker. Trim dims the full track and adds a bright trim polyline. Tack mode replaces the polyline with coloured segments (port=red, stbd=green). Wind barbs are Leaflet DivIcons with inline SVG, rotated by TWD, scaled by zoom level.
- **Beating** (`analysis-container`): Canvas scatter plot of BSP vs TWA (folded, 0–55°) for all upwind data in trim window.
- **TWD** (`twd-container`): Table of tack events detected via TWA sign changes; shows port/stbd TWD and the shift between them. Rows with unstable wind (range >10° in ±30–60s windows) are highlighted red.

### TWD/tack detection logic

`detectTacks()` in `app.js`: finds TWA sign changes (excluding |TWA|≥90°), interpolates zero-crossing time, looks up average TWD in 30–60s windows before/after the tack, flags as unstable if the TWD range in either window exceeds 10°.
